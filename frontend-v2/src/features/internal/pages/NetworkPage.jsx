...
import { useRedeExpenses } from '../hooks/useExpenses';
import {
  createRedeDesobstrucao,
  createRedeReparacao,
  createCaixaDesobstrucao,
  createCaixaReparacao,
  createCaixaReparacaoTampa,
...
const TABS = [
  { label: 'Despesas' },
  { label: 'Desobstrução Rede' },
  { label: 'Reparação / Colapso Rede' },
  { label: 'Desobstrução Caixas' },
  { label: 'Reparação Caixas' },
  { label: 'Reparação Tampa Caixas' },
];
...
const NetworkPage = () => {
  ...
  return (
    ...
    >
      ...
      {tab === 0 && (
        <ExpensePage useHook={useRedeExpenses} color={color} />
      )}
      {tab === 1 && (
        <RequestForm
          title="Registo de Desobstrução de Rede"
          submitFn={createRedeDesobstrucao}
          associates={associates}
        />
      )}
      {tab === 2 && (
        <RequestForm
          title="Registo de Reparação / Colapso de Rede"
          submitFn={createRedeReparacao}
          associates={associates}
        />
      )}
      {tab === 3 && (
        <RequestForm
          title="Registo de Desobstrução de Caixas"
          submitFn={createCaixaDesobstrucao}
          associates={associates}
        />
      )}
      {tab === 4 && (
        <RequestForm
          title="Registo de Reparação de Caixas"
          submitFn={createCaixaReparacao}
          associates={associates}
        />
      )}
      {tab === 5 && (
        <RequestForm
          title="Registo de Reparação de Tampa de Caixas"
          submitFn={createCaixaReparacaoTampa}
          associates={associates}
        />
      )}
    </ModulePage>
