  ...
  const formContent = (
    <>
      ...
      <Box component="form" onSubmit={handleSubmit}>
        <Stack spacing={3}>
          <TextField
            fullWidth label="Nome Completo" autoComplete="name" autoFocus
            value={formData.name} onChange={(e) => updateField('name', e.target.value)}
            error={hasError('name')} helperText={getFieldError('name')}
            disabled={isLoading} sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }}
          />
          <TextField
            fullWidth label="Email" type="email" autoComplete="email"
            value={formData.email} onChange={(e) => updateField('email', e.target.value)}
            error={hasError('email')} helperText={getFieldError('email')}
            disabled={isLoading} sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }}
          />
          <TextField
            fullWidth label="Palavra-passe" type="password" autoComplete="new-password"
            value={formData.password} onChange={(e) => updateField('password', e.target.value)}
            error={hasError('password')}
            helperText={getFieldError('password') || 'Mínimo 6 caracteres com maiúsculas, minúsculas e números'}
            disabled={isLoading} sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }}
          />
          <TextField
            fullWidth label="Confirmar Palavra-passe" type="password" autoComplete="new-password"
            value={formData.confirmPassword} onChange={(e) => updateField('confirmPassword', e.target.value)}
            error={hasError('confirmPassword')} helperText={getFieldError('confirmPassword')}
            disabled={isLoading} sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }}
          />
          <Typography variant="caption" color="text.secondary">
            Ao criar uma conta, concorda com os nossos{' '}
            <Link href="#" underline="hover" sx={{ color: 'primary.main' }}>Termos de Serviço</Link>
            {' '}e{' '}
            <Link href="#" underline="hover" sx={{ color: 'primary.main' }}>Política de Privacidade</Link>
          </Typography>
          <Button
      ...
      <Box sx={{ textAlign: 'center' }}>
        <Typography variant="body2" color="text.secondary">
          Já tem uma conta?{' '}
          <Link component={RouterLink} to="/login" variant="body2" fontWeight="bold" underline="hover" sx={{ color: 'primary.main' }}>
            Entrar
          </Link>
        </Typography>
      </Box>
