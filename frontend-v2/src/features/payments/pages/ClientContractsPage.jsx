...
import {
  ...
  Divider, Tooltip, useTheme, alpha, Badge,
...
const emptyContract = {
  ts_entity: '', start_date: '', stop_date: '', family: '',
  tt_contractfrequency: '', address: '', postal: '', door: '',
  floor: '', nut1: '', nut2: '', nut3: '', nut4: '',
};
...
const useContracts = () =>
  useQuery({
    queryKey: ['contracts'],
    queryFn: () => paymentService.getContracts?.() ?? import('@/services/api/client').then(m => m.default.get('/payments/contracts')),
    staleTime: 3 * 60 * 1000,
...
const ClientContractsPage = () => {
  ...
  const { data: contractPayments = [], isLoading: isLoadingPayments } = useContractPayments(selectedContract?.pk);
  const { data: alerts = [], isLoading: isLoadingAlerts, refetch: refetchAlerts } = useContractAlerts();
  ...
  // ── Handlers NIF ────────────────────────────────────────────────────────────
  const resetNifDialog = () => { setNifInput(''); setNifEntity(null); setNifError(''); };
  ...
  const handleNifSearch = async () => {
    ...
    try {
      ...
      if (!entity?.pk) { setNifError('NIF não encontrado'); return; }
      setNifEntity(entity);
      setContractForm({
        ...emptyContract,
        ts_entity: entity.pk,
  ...
  const columns = [
    ...
    {
      field: 'start_date', headerName: 'Início', width: 100,
      valueFormatter: (v) => fmtDate(v),
    },
    {
      field: 'stop_date', headerName: 'Fim', width: 100,
      valueFormatter: (v) => fmtDate(v),
    },
    ...
    { field: 'nut3', headerName: 'Freguesia', flex: 1, minWidth: 160, valueFormatter: (v) => v || '—' },
  ...
  // ── Data fim validation ──────────────────────────────────────────────────────
  const stopDateError = contractSubmitted && (
    !contractForm.stop_date ? 'Campo obrigatório' :
  ...
  return (
        ...
        <Box sx={{ display: 'flex', gap: 1 }}>
          ...
          <Tooltip title="Alertas de pagamento">
            <Badge badgeContent={overdueCount} color="error">
              <Button
                variant="outlined" size="small" startIcon={<AlertsIcon />}
    ...
    >
      ...
      <Paper sx={{ borderRadius: 2 }}>
        <Box sx={{ p: 2 }}>
          <TextField
            size="small" placeholder="Pesquisar por cliente, NIPC, morada, localização..."
      ...
      <Dialog open={nifDialogOpen} onClose={() => { setNifDialogOpen(false); resetNifDialog(); }} maxWidth="xs" fullWidth>
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={handleNifSearch} disabled={nifLoading || nifInput.length < 9}>
      ...
      <Dialog open={contractDialogOpen} onClose={() => { setContractDialogOpen(false); setContractForm(emptyContract); setNifEntity(null); setContractSubmitted(false); }} maxWidth="md" fullWidth>
        <DialogTitle>Novo Contrato</DialogTitle>
        <DialogContent>
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            <Grid size={{ xs: 12, sm: 6 }}>
              <TextField
                label="Entidade" value={nifEntity?.name || ''} fullWidth size="small"
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              <TextField
                label="Família" type="number" fullWidth size="small"
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              <FormControl fullWidth size="small" error={contractSubmitted && !contractForm.tt_contractfrequency}>
                ...
                {contractSubmitted && !contractForm.tt_contractfrequency && <FormHelperText>Campo obrigatório</FormHelperText>}
              </FormControl>
            </Grid>
            <Grid size={{ xs: 12, sm: 6 }}>
              <TextField
                type="date" label="Data início *" fullWidth size="small"
                value={contractForm.start_date}
                onChange={(e) => setContractForm(f => ({ ...f, start_date: e.target.value }))}
                InputLabelProps={{ shrink: true }}
                error={contractSubmitted && !contractForm.start_date}
                helperText={contractSubmitted && !contractForm.start_date ? 'Campo obrigatório' : ''}
              />
            </Grid>
            <Grid size={{ xs: 12, sm: 6 }}>
              <TextField
                type="date" label="Data fim *" fullWidth size="small"
        ...
        <DialogActions>
          <Button onClick={() => { setContractDialogOpen(false); setContractForm(emptyContract); setNifEntity(null); setContractSubmitted(false); }}>
            Cancelar
          </Button>
          <Button variant="contained" disabled={isCreating}
            onClick={() => {
              setContractSubmitted(true);
              const { ts_entity, start_date, stop_date, family, tt_contractfrequency } = contractForm;
                ...
                {isLoadingPayments ? (
                  ...
                  <TableRow key={cp.pk} hover>
                    ...
                    <TableCell align="right">
                      <Typography fontWeight={600}>€{Number(cp.value || 0).toFixed(2)}</Typography>
                    </TableCell>
                    {/* Faturado */}
                    <TableCell>
                        ...
                        {!cp.presented && (
                          <Tooltip title="Registar data de faturação">
                            <IconButton size="small" color="primary"
                              onClick={() => { setPaymentToInvoice(cp); setInvoiceDate(''); setInvoiceDialogOpen(true); }}
                            >
                        ...
                        {!cp.payed && (
                          <Tooltip title="Validar pagamento">
                            <IconButton size="small" color="success"
                              onClick={() => { setPaymentToValidate(cp); setPayedDate(''); setValidateDialogOpen(true); }}
                            >
                              <CheckCircleIcon fontSize="small" />
                            </IconButton>
                          </Tooltip>
        ...
        <DialogTitle>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, flexWrap: 'wrap' }}>
            ...
            >
              ...
              <ToggleButton value="pending" sx={{ color: 'warning.main' }}>Dentro do prazo ({alerts.length - overdueCount})</ToggleButton>
            </ToggleButtonGroup>
            <Tooltip title="Atualizar">
          ...
          {isLoadingAlerts ? (
            ...
            <TableContainer>
              <Table size="small">
                ...
                <TableBody>
                  {alerts.filter(a => alertsFilter === 'all' || (alertsFilter === 'overdue' ? a.overdue : !a.overdue)).map((a) => (
                    <TableRow key={a.pk} hover sx={{ bgcolor: a.overdue ? alpha(theme.palette.error.main, 0.05) : 'inherit' }}>
                      ...
                      <TableCell>{fmtDate(a.start_date)} → {fmtDate(a.stop_date)}</TableCell>
                      ...
                      <TableCell>
                        <Tooltip title="Registar pagamento">
                          <IconButton size="small" color="success"
                            onClick={() => { setPaymentToValidate(a); setPayedDate(''); setValidateDialogOpen(true); }}
                          >
                            <CheckCircleIcon fontSize="small" />
                          </IconButton>
                        </Tooltip>
      ...
      <Dialog open={invoiceDialogOpen} onClose={() => setInvoiceDialogOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          ...
          <TextField
            type="date" label="Data de faturação" fullWidth
        ...
        <DialogActions>
          ...
          <Button variant="contained" disabled={!invoiceDate || isInvoicing}
            onClick={() => invoicePayment({ contractPk: selectedContract.pk, paymentPk: paymentToInvoice.pk, invoiceDate })}
          >
      ...
      <Dialog open={validateDialogOpen} onClose={() => setValidateDialogOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          ...
          <TextField
            type="date" label="Data do pagamento" fullWidth
            value={payedDate} onChange={(e) => setPayedDate(e.target.value)}
            InputLabelProps={{ shrink: true }}
          />
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setValidateDialogOpen(false)}>Cancelar</Button>
          <Button variant="contained" disabled={!payedDate || isValidating}
            onClick={() => validatePayment({ contractPk: paymentToValidate.tb_contract ?? selectedContract?.pk, paymentPk: paymentToValidate.pk, payedDate })}
          >
