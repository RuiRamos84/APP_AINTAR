...
const PaymentStatus = ({ transactionId, status: paymentResult, onComplete }) => {
    ...
    return (
        <Box sx={{ textAlign: 'center', p: 4 }}>
            ...
            <Button variant="contained" onClick={() => onComplete?.({ status: paymentResult || 'SUCCESS', transactionId })} startIcon={<Euro />} sx={{ background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', px: 4 }}>
                Concluir
            </Button>
        </Box>
    );
