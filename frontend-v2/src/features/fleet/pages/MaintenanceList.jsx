...
import { Box, Typography, Chip, Stack, Tooltip, Select, MenuItem, TextField } from '@mui/material';
...
const MaintenanceList = ({
        ...
        renderCell: ({ row }) => (
          <Cell>
            {row.tt_maintenancetype_pk === BREAKDOWN_TYPE_PK ? (
              ...
              <Chip
                label={row.status ?? 'Resolvida'}
                size="small"
                color={getStatusColor(row.ts_maintenancestatus ?? 3)}
              />
            )}
          </Cell>
        ),
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      ...
      <MaintenanceFormModal open={isModalOpen} onClose={onCloseModal} />
