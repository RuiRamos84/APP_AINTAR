// features/telemetry/components/SensorMap.jsx
...
// Ajusta o mapa para mostrar todos os sensores
...
// Marcador individual com popup de acções
function SensorMarker({ sensor, sensorEntry, isFiltered, canSelect, onSelect, onDeselect }) {
  const markerRef = useRef(null);
  const isSelected = !!sensorEntry;
  const color = isSelected ? sensorEntry.color : isFiltered ? '#9e9e9e' : '#1976d2';
            ...
            {isSelected && (
              <div
                style={{
                  ...
                  background: sensorEntry.color, flexShrink: 0,
                }}
              />
            )}
...
/**
 * Mapa principal com todos os sensores
 */
...
/**
 * Mini mapa para a vista de análise (sidebar)
 */
export function MiniSensorMap({ allSensors = [], selectedSensors = [], width = 200, height = 130 }) {
  const entries = selectedSensors
    ...
    .filter((e) => e.sensor?.latitude != null && e.sensor?.longitude != null);
