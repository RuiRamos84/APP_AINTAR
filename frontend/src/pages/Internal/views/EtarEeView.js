...
import {
    ...
    Chip, Checkbox, FormControlLabel, CircularProgress, Divider, InputAdornment
} from "@mui/material";
...
const EtarEeView = ({ areaId }) => {
    ...
    const handleLocationChange = (event) => {
        ...
        // Auto-mapear para município correspondente
        if (location && metaData?.associates) {
            ...
            if (matchedAssociate) {
                // Actualizar estado do mass request se estiver activo
                // (será usado pelo MassRequestComponent)
    ...
    const MassRequestComponent = ({ areaId, requestType, title, selectedLocation }) => {
        ...
        // Auto-sincronizar com localização seleccionada
        React.useEffect(() => {
            if (selectedLocation && metaData?.associates) {
                ...
                if (matchedAssociate && matchedAssociate.pk !== selectedAssociate) {
                    ...
                    setSelectedItems([]); // Reset items when municipality changes
                }
            }
        ...
        const getFilteredEntities = () => {
            ...
            // Mapear nome do município para localização
        ...
        // Função para sincronizar com localização externa
        const syncWithLocation = (location) => {
            if (location && metaData?.associates) {
                const matchedAssociate = metaData.associates.find(associate =>
                    associate.name === `Município de ${location}` ||
                    associate.name.replace("Município de ", "") === location
                                ...
                                <CardContent>
                                    <Box display="flex" alignItems="center">
                                        ...
                                        <Box>
                                            ...
                                            {isDisabled && (
                                                <Typography variant="caption" color="text.secondary">
                                                    Seleccione uma {areaId === 1 ? "ETAR" : "EE"}
                                                </Typography>
                                            )}
                                        </Box>
                                    </Box>
