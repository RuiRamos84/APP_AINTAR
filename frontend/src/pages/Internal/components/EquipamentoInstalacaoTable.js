...
const EquipamentoModal = ({ open, onClose, editing, tbInstalacao, metaData, onSaved }) => {
    const [form, setForm] = useState(EMPTY_FORM);
    const [loading, setLoading] = useState(false);
    const isValid = form.tt_equiptipo && form.tt_equiplocalizacao && form.modelo?.trim() && form.serial?.trim();

    const equipTipos = metaData?.equiptipo || [];
    const equipLocs = metaData?.equiplocalizacao || [];

    useEffect(() => {
        if (editing) {
            setForm({
                tt_equiptipo: editing.tt_equiptipo ?? "",
                tt_equiplocalizacao: editing.tt_equiplocalizacao ?? "",
                marca: editing.marca ?? "",
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!form.tt_equiptipo || !form.tt_equiplocalizacao || !form.modelo || !form.serial) {
            notifyError("Tipo, Localização, Modelo e Nº de Série são obrigatórios");
            return;
        }
        setLoading(true);
        try {
                    ...
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={12} sm={6}>
                            <FormControl fullWidth>
                                ...
                                <Select
                                    value={form.tt_equiplocalizacao}
                                    onChange={(e) => set("tt_equiplocalizacao", e.target.value)}
                                    label="Localização *"
                                >
                                    {equipLocs.map((l) => (
                        ...
                        <Grid item xs={12} sm={6}>
                            <TextField
                                fullWidth size="small" label="Data de Instalação"
                                type="date"
...
const EquipamentoInstalacaoTable = ({ selectedEntity, metaData }) => {
    ...
    return (
        <Box>
                    ...
                    <Table size="small">
                        <TableHead>
                            <TableRow>
                                ...
                                <TableCell><b>Data Inst.</b></TableCell>
                        ...
                        <TableBody>
                            {rows.map((row) => (
                                <TableRow key={row.pk} hover>
                                    <TableCell>{getLabel(equipTipos, row.tt_equiptipo)}</TableCell>
            ...
            <EquipamentoModal
                ...
                editing={editing}
                tbInstalacao={selectedEntity?.pk}
                metaData={metaData}
                onSaved={load}
            />
