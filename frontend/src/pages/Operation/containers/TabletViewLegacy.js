...
import CompletionWizardModal from '../modals/CompletionWizardModal/CompletionWizardModal';
import { completeOperatorTask, validateTaskCompletion } from '../services/completionService';
import { getColumnsForView, getRemainingDaysColor, getUserNameByPk } from '../utils/helpers';
...
const TabletViewLegacy = ({
    ...
    // UI State - usando useState em vez de store.getUI()
    const [viewMode, setViewMode] = useState('grid');
    const [detailsDrawer, setDetailsDrawer] = useState(false);
    const [selectedItem, setSelectedItem] = useState(null);
    const [actionDrawer, setActionDrawer] = useState(false);
    const [completeDialogOpen, setCompleteDialogOpen] = useState(false);
    const [wizardDialogOpen, setWizardDialogOpen] = useState(false); // Novo wizard modal
    const [filterDrawer, setFilterDrawer] = useState(false);
    const [completionNote, setCompletionNote] = useState('');

    // Filter State - usando useState em vez de store.getFilters()
    ...
    const handleCompleteProcess = () => {
        ...
        if (isFossaView) {
            // Para Fossa: Usar wizard com parâmetros + nota
            setWizardDialogOpen(true);
        } else {
            // Para outros: Usar modal simples só com nota
    ...
    const handleFinalCompletion = async (attachment = null) => {
        ...
        try {
            // Passar selectedItem (já inclui 'what' e 'tt_type' vindos da view) e metaData
            await completeOperatorTask(
                selectedItem.pk,
                completionNote,
                selectedItem,      // Document da view (inclui what e tt_type)
                globalMetaData     // MetaData global com step_transitions
            );
    ...
    const handleWizardComplete = (documentId) => {
        // Callback do wizard após conclusão bem sucedida
        if (onTaskCompleted) {
            onTaskCompleted(documentId);
        }
    ...
    return (
        <Box sx={{ height: "100vh", display: "flex", flexDirection: "column", bgcolor: 'background.default' }}>
            ...
            {/* Novo Wizard Modal para Fossa (Parâmetros + Nota + Foto) */}
            ...
            {/* Modal simples para outros tipos (só Nota + Foto) */}
            <CompletionModal
...
export default TabletViewLegacy;
