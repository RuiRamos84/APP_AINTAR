...
export function MiniSensorMap({ allSensors = [], selectedSensors = [], onClick, width = 200, height = 130 }) {
...
function SensorMarker({ sensor, sensorEntry, isFiltered, canSelect, onSelect, onDeselect, onViewRecords }) {
    const markerRef = useRef(null);
    const isSelected = !!sensorEntry;
    const markerColor = isSelected ? sensorEntry.color : isFiltered ? "#9e9e9e" : "#1976d2";
                ...
                <div style={{ minWidth: 190, fontFamily: "sans-serif" }}>
                    ...
                    {isSelected ? (
                        <button
                            ...
                            style={{
                                padding: "6px 12px", background: "#ef5350", color: "#fff",
                        ...
                        <button
                            ...
                            style={{
                                padding: "6px 12px", background: "#43a047", color: "#fff",
                                border: "none", borderRadius: 4, cursor: "pointer",
                                fontSize: 13, width: "100%", marginBottom: 4,
                            }}
                        >
                    ...
                    {onViewRecords && (
                        <button
                            onClick={() => onViewRecords?.(sensor.name)}
                            style={{
                                padding: "5px 12px", background: "transparent", color: "#1976d2",
                                border: "1px solid #1976d2", borderRadius: 4, cursor: "pointer",
...
export default function SensorMap({
            ...
            >
                {isLoading ? (
                    ...
                    <>
                        ...
                        <Typography color="text.secondary">Nenhum sensor com coordenadas disponíveis.</Typography>
                    </>
                )}
            </Box>
        );
    ...
    return (
        <Box
            sx={{
                width: "100%", height: "calc(100vh - 260px)", minHeight: 400, borderRadius: 2, overflow: "hidden",
        ...
        >
            <MapContainer center={center} zoom={7} style={{ height: "100%", width: "100%" }} scrollWheelZoom>
                ...
                {validSensors.map((sensor) => {
                    ...
                    const isFiltered = !!(selectedType && sensor.tt_sensortype !== selectedType);
                    return (
                        <SensorMarker
                            key={sensor.name}
                            sensor={sensor}
