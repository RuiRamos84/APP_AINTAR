export default {
  ...
  db: { readView: 'vbl_equipamento', writeView: 'vbf_equipamento', pkField: 'pk' },
  ...
  formView: {
    sections: [
      { title: 'Identificação', fields: ['tt_equipamento$tipo', 'marca', 'modelo', 'serial'], cols: 2 },
      { title: 'Financeiro',    fields: ['valor'], cols: 2 },
    ],
  },

  relations: {
    alocacoes: { type: 'hasMany', entity: 'equipamento_aloc',   foreignKey: 'tb_equipamento', label: 'Alocações',  icon: 'MapPin' },
    reparacoes:{ type: 'hasMany', entity: 'equipamento_repair', foreignKey: 'tb_equipamento', label: 'Reparações', icon: 'Hammer' },
  },
  detailView: { tabs: [
